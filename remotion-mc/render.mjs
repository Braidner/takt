...
const serveUrl = await bundle({ entryPoint: path.resolve('remotion-mc/index.tsx'), onProgress: () => {} });
const composition = await selectComposition({ serveUrl, id: 'McMovie', ...opts });
console.log(`композиция: ${composition.width}×${composition.height}, `
  + `${composition.durationInFrames} кадров, ${composition.fps} к/с`);

const out = 'studio/journal/projects/mc-медиа/movie-remotion.mp4';
let last = 0;
await renderMedia({
