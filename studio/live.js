 ...
 * Сторож тишины.
 *
 * Оборванный поток браузер замечает не всегда: сокет остаётся открытым, данные не
 * идут, а на экране всё как обычно — это и есть «зависло». Поэтому связь считается
 * живой не по состоянию соединения, а по тому, что из него приходит: студия шлёт
 * пульс каждые тридцать секунд, и тишина дольше сорока пяти означает обрыв, кто бы
 * что ни думал о сокете.
 */
const ТИШИНА = 45_000;
let сторож = null;
...
function connect() {
  ...
  stream.onopen = () => { задержкаСвязи = 1000; слышим(); };
