...
try {
  ...
  /* Ждать декодирования снимков обязан только привод вывода: он снимает кадр и не
     может застать картинку серой. Человеку в студии ждать нечего — там кадр дорисуется
     сам, а ожидание стоило дорого: decode() в неактивной вкладке откладывается до
     показа, и врезка упиралась в восьмисекундный таймаут на КАЖДОЙ перезагрузке.
     Правка титра подвешивала студию на восемь секунд — это и ощущалось как «зависло,
     потом отвисло». */
  if (render) {
    await Promise.race([
