...
test('новый план в середине не перенумеровывает соседей', () => {
  /* Идентификатор связывает план со снятым состоянием, эффектами и замечаниями.
     Пока свободный номер искали среди уже просмотренных планов, вставка третьим
     забирала p03 у того, кто им был, — и дальше по цепочке. Снятый материал
     оставался в states под старыми именами, и половина ролика становилась
     «не снята» одним движением мыши. */
  const было = normalizeStoryboard({
    title: 'Демо',
    plans: [
      { id: 'p01', title: { text: 'Раз' } },
      { id: 'p02', title: { text: 'Два' } },
      { id: 'p03', title: { text: 'Три' } },
    ],
  });
  const стало = normalizeStoryboard({
    ...было,
    plans: [было.plans[0], было.plans[1],
            { mode: 'insert', insert: { src: 'i.html' }, title: { text: 'Вставка' } },
            было.plans[2]],
  });
  assert.deepEqual(стало.plans.map((p) => p.title.text), ['Раз', 'Два', 'Вставка', 'Три']);
  assert.deepEqual(стало.plans.map((p) => p.id), ['p01', 'p02', 'p04', 'p03']);
});

test('живой план длится столько, сколько снято', () => {
  /* Снятое движение нельзя растянуть: плёнка берёт длину из записи. Пока раскадровка
     считала его по назначенному времени, всё, что идёт следом, стояло на разнице
     раньше картинки — клик по плану уводил мимо, замечание по таймкоду приезжало
     агенту с чужим временем, реплика ложилась не под свой титр. Тот же дефект,
     что был у заставки, только источник другой. */
  const состояния = [{ id: 'p02', mode: 'live', video: 'states/p02.mp4', seconds: 3.2 }];
  const sb = normalizeStoryboard({
    title: 'Демо', slate: false, end: false,
    plans: [
      { id: 'p01', title: { text: 'Раз' }, action: { kind: 'hold', seconds: 2 } },
      { id: 'p02', mode: 'live', title: { text: 'Живой' },
        duration: { source: 'manual', seconds: 8 } },
      { id: 'p03', title: { text: 'Три' }, action: { kind: 'hold', seconds: 2 } },
    ],
  }, состояния);

  assert.equal(sb.plans[1].duration.seconds, 3.2, 'длину задаёт запись, а не человек');
  assert.equal(sb.plans[1].duration.source, 'shot');
  assert.equal(sb.plans[2].at, sb.plans[1].at + 3.2, 'следующий план встык к записи');
});

test('без снятой записи живой план держит назначенное время', () => {
  // Раскадровку утверждают до съёмки: тогда взять длину неоткуда, и назначенное
  // человеком остаётся единственным, что о плане известно.
  const sb = normalizeStoryboard({
    title: 'Демо', slate: false, end: false,
    plans: [{ id: 'p01', mode: 'live', title: { text: 'Живой' },
              duration: { source: 'manual', seconds: 8 } }],
  });
  assert.equal(sb.plans[0].duration.seconds, 8);
});
